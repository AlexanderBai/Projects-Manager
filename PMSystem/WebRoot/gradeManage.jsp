...
<input type="hidden" id="gradePageCount"value="${gradePageCount}">
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">		
							...
							<tbody>
								<c:forEach items="${pageAll.list}" var="item" varStatus="status">
									<tr>
										...
										<td>${item.id}</td>
										<td>${item.name }</td>
										<td>${item.businessgrade }</td>
										<td>${item.behaviorgrade }</td>
										<td>
											<div>
        ...
        <td width="60%">
            <a href="${pageContext.request.contextPath }/GradeManageServlet?op=page&currentPageIndex=${pageAll.currentPageIndex -1}" style="text-decoration:none;">|&lt;</a>
            <c:forEach begin="${startIndex}" end="${endIndex }" var="n">
                <a href="${pageContext.request.contextPath }/GradeManageServlet?op=page&currentPageIndex=${n}">${pageAll.currentPageIndex == n ?"<font color=red>":"<font>"}${n }</font></a>
            </c:forEach>
            <a href="${pageContext.request.contextPath }/GradeManageServlet?op=page&currentPageIndex=${pageAll.currentPageIndex +1}" style="text-decoration:none;">&gt;|</a>
        </td>
        <td width="10%">
